...
let rec write_node output (t : graph_terminal) =
  ...
  | Dictionary (l, _) -> List.iter (write_node output) l
...
let rec obtain_edges (edges : (int, int) Hashtbl.t)
  ...
  let add_id = match t with Leaf _ -> true | Null _ -> true | _ -> false in
  ( match previous with
  | Some x ->
  ...
  | Dictionary (l, _) ->
      let associations =
        List.map
          (obtain_edges edges
             (match previous with None -> Some [this_id] | Some x -> Some x))
          l
      in
      Some
        ( List.concat
